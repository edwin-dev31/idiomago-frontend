    ...
    <div className="flex flex-col items-center md:items-start text-center md:text-left gap-5">
      <div className="flex flex-col items-center md:items-start gap-3 md:pl-10">
        <Avatar className="w-40 h-40 ring-4 ring-blue-500 shadow-lg">
          <AvatarImage
            src={imageSrc || user?.imageUrl || `https://api.dicebear.com/7.x/fun-emoji/svg?seed=${user?.username}`}
            alt={user?.username}
          />
        ...
        <div className="flex gap-2">
          ...
          {file && <Button onClick={handleSubmit} disabled={uploading}>{uploading ? "Uploading..." : "Upload"}</Button>}
          {imageSrc && <Button variant="ghost" onClick={clear}>Cancel</Button>}
        </div>
      </div>

      <h1 className="text-5xl font-extrabold text-[#0D2B3E] leading-tight w-full md:pl-10 dark:text-white">
