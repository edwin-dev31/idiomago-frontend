...
const SaveCustomWordForm: React.FC<Props> = ({ onSearch }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md  dark:border-gray-100 w-full max-w-[550px] border">
      ...
      <div className="flex flex-col md:grid md:grid-cols-2 gap-2 mb-4">
        <label className="text-sm block mb-1 text-blue-900">Language *</label>
        <SingleSelector
      ...
      <div className="flex flex-col md:grid md:grid-cols-2 gap-2 mb-4">
        <label className="text-sm block mb-1 text-blue-900">Visibility *</label>
        <RadioGroup
          value={visibility}
          onValueChange={(val) => setVisibility(val as Visibility)}
          className="flex flex-nowrap gap-4 w-full justify-center"
        >
